...
- `web-controllers`: Enables HTTP controllers. Requires importing `sword::web::*`.
- `socketio-controllers`: Enables real-time controllers. Requires importing `sword::socketio::*`.
...
- `grpc-controllers`: Enables gRPC controllers based on `tonic`. Requires importing `sword::grpc::*`.
...
- Message limits via `[application.body-limit]`.
