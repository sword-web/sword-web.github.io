...
This page describes the general model for handling errors in Sword. The details of each transport live in the practical guides of each one.

## Principles

- Errors are typed as enums, not as strings.
- The format returned to clients is uniform within each protocol.
- Only what each variant declares is exposed; internal details are not exposed.
...
The natural design of `thiserror` is agnostic, so Sword provides abstractions to transform the variants into transport errors without coupling the domain to them.
...
| Transport | Macro       | Result          | Documentation                                       |
| --------- | ----------- | --------------- | --------------------------------------------------- |
| Web       | `HttpError` | `JsonResponse`  | See [here](/en/practical-guides/web/error-handling) |
| gRPC      | `GrpcError` | `tonic::Status` | See [here](/en/practical-guides/grpc/grpc-errors)   |
